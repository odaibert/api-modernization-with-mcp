...
|  |  |  • Subscription: not reqd   |    |  • Policy: set-backend-service,  |        |     |
|  |  |  • Backend: Container App   |    |    token metrics, auth header    |        |     |
|  |  +--------------+--------------+    +----------------+-----------------+        |     |
|  |                 |                                    |                           |    |
|  |  +--------------+-------------------+   +-----------+-------------------+      |      |
|  |  | Subscription: not required       |   | Subscription: api-key header  |      |      |
...
|  |  • Admin auth enabled + UAI      |    |  • Scaling: 1–3 replicas         |            |
|  +----------------------------------+    +----------------------------------+            |
|                                                                                          |
|  +---------------------------------------------------------------------------------+     |
|  |  Observability Layer                                                            |     |
...
|  +----------------------------------------------------------------+  |
|  | * Inference API: api-key subscription header (required)        |  |
|  | * MCP API: subscription not required (open by default)         |  |
...
resource mcp 'Microsoft.ApiManagement/service/apis@2024-06-01-preview' = {
  properties: {
    ...
    subscriptionRequired: false    // No api-key needed (open by default)
    subscriptionKeyParameterNames: {
      header: 'api-key'            // Accepted but not enforced
    }
  }
}
```
...
| **MCP API: no subscription required** | Simplifies the demo — MCP endpoint is open by default. Inference API still requires `api-key` | Production should enable `subscriptionRequired: true` or add OAuth 2.0 / JWT validation for MCP |
